...
# We Manage Yum for Centos/RedHat Linux only
  case $::operatingsystem {
    ...
    centos: {
      ...
      if $extrarepo =~ /centos-testing/ { include yum::repo::centos_testing }
      if $extrarepo =~ /epel/ { include yum::repo::epel }
      if $extrarepo =~ /rpmforge/ { include yum::repo::rpmforge }
      if $extrarepo =~ /karan/ { include yum::repo::karan }
      if $extrarepo =~ /jpackage/ { include yum::repo::jpackage }
      if $extrarepo =~ /remi/ { include yum::repo::remi }
      if $extrarepo =~ /tmz/ { include yum::repo::tmz }
      if $extrarepo =~ /puppetlabs/ { include yum::repo::puppetlabs }
      if $extrarepo =~ /omd/ { include yum::repo::omd }
      if $extrarepo =~ /spacewalk-client/ { include yum::repo::spacewalk-client }
